...
contract SmartChefFactory is Ownable {
    ...
    address service;
    uint256 priceToMakePoolEth = 500_000_000_000;
    uint256 priceToMakePoolTokens = 5000;
    IBEP20 public tokenToPayIn;

    constructor(IBEP20 _tokenToPayIn, address _service) {
    ...
    function deployPool(
        ...
        //if _payInTokens is true then pay in tokens if not then pay in eth
        if(_payInTokens){
          console.log("Pay in tokens");
          uint256 allow = tokenToPayIn.allowance(address(msg.sender), address(this));
          console.log("Allowance", allow);

          tokenToPayIn.transferFrom(address(msg.sender), service, 1200);
        } else {
            console.log("Pay in Eth");
            payable(service).transfer(priceToMakePoolEth);
        }
    ...
    function deployNFTPool(
        ...
        //if _payInTokens is true then pay in tokens if not then pay in eth
        if(_payInTokens){
           //ask to approve on front end
          tokenToPayIn.safeTransfer(service, priceToMakePoolTokens);
        } else {
            payable(service).transfer(priceToMakePoolEth);
        }
    ...
    function changePoolPrice(uint256 _priceEth, uint256 _priceTokens) external onlyOwner { 
        priceToMakePoolEth = _priceEth;
        priceToMakePoolTokens = _priceTokens;
    }
}
