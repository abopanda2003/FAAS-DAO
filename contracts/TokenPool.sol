...
contract SmartChefInitializable is Ownable, ReentrancyGuard {
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if(rewardToken == stakedToken) {
            stakedTotal = stakedTotal.sub(_amount).sub(pending);
        }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if(stakedToken == rewardToken){
            stakedTokenSupply = stakedToken.balanceOf(address(this)) - stakedTotal;
        } else {
    ...
    function _updatePool() internal {
        ...
        uint256 stakedTokenSupply = stakedToken.balanceOf(address(this)) - stakedTotal;
